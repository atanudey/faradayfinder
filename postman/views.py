...
from .forms import WriteForm, AnonymousWriteForm, QuickReplyForm, FullReplyForm, FullForwardForm
from .models import Message, get_order_by
from .utils import format_subject, format_subject_forward, format_body, format_body
...
class InboxView(FolderMixin, TemplateView):
    ...
    view_name = 'inbox'
    # for TemplateView:
    template_name = 'o/message_inbox.html'
...
class SentView(FolderMixin, TemplateView):
    ...
    view_name = 'sent'
    # for TemplateView:
    template_name = 'o/message_sent.html'
...
class ArchivesView(FolderMixin, TemplateView):
    ...
    view_name = 'archives'
    # for TemplateView:
    template_name = 'o/message_archives.html'
...
class TrashView(FolderMixin, TemplateView):
    ...
    view_name = 'trash'
    # for TemplateView:
    template_name = 'o/message_trash.html'
...
class WriteView(ComposeMixin, FormView):
    ...
    template_name = 'o/message_write.html'
...
class ReplyView(ComposeMixin, FormView):
    ...
    template_name = 'o/message_reply.html'
...
class ForwardView(ComposeMixin, FormView):
    """
    ...
    Optional attributes:
        ...
        + those of ComposeMixin
    """
    form_class = FullForwardForm
    formatters = (format_subject_forward, format_body)
    autocomplete_channel = None
    template_name = 'o/message_forward.html'
    ...
    def dispatch(self, request, message_id, *args, **kwargs):
        perms = Message.objects.perms(request.user)
        self.parent = get_object_or_404(Message, perms, pk=message_id)
        self.parent.forward = True  # This will help identify if the new message is a forward message

        body_prefix = "\n---------- Forwarded message ----------\n"
        body_prefix += "From: " + self.parent.sender.first_name + " " + self.parent.sender.last_name + " <" + self.parent.sender.email + ">\n"
        body_prefix += "Date: " + date_format(timezone.localtime(self.parent.sent_at), "DATETIME_FORMAT") + "\n"
        body_prefix += "Subject: " + self.parent.subject + "\n"
        body_prefix += "To: " + self.parent.recipient.first_name + " " + self.parent.recipient.last_name + " <" + self.parent.recipient.email + ">\n" 
        
        self.parent.body = body_prefix + "\n" + self.parent.body        
        return super(ForwardView, self).dispatch(request,*args, **kwargs)
    ...
    def get_form_kwargs(self):
        kwargs = super(ForwardView, self).get_form_kwargs()
        kwargs['channel'] = self.autocomplete_channel
        if self.request.method == 'POST':
            if 'subject' not in kwargs['data']:  # case of the quick reply form
                post = kwargs['data'].copy()  # self.request.POST is immutable
    ...
    def get_context_data(self, **kwargs):
        context = super(ForwardView, self).get_context_data(**kwargs)
        context['recipient'] = self.parent.obfuscated_sender
        return context
...
class DisplayMixin(NamespaceMixin, object):
    ...
    template_name = 'o/message_view.html'
...
class DeleteforeverView(UpdateMessageMixin, View):
    """Mark messages/conversations as archived."""
    field_bit = 'delete_forever'
    success_msg = ugettext_lazy("Messages or conversations permanently deleted.")
    field_value = True
