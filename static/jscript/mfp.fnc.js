...
/*
	function check_org_protection(main_cat,name)
	{	// function will be called only if the organization is password protected
		// if session is not started
		jQuery('#org_name').html('<em>&#147;&nbsp;'+name+'&nbsp;&#148;</em>');
		
		  jQuery.ajax({
		  type : 'POST',
		  url  : AJAX_PATH+"ajx_check_organization_password.php?main_cat="+main_cat,
		  cache: false,	
		  data : "&exist=1&rand="+ Date(),
		  success : function(res){ 
				if(res != 0) { window.location.href="view-organization/"+res; } 
				else{ 
...
*/
 ...
 function popup_ticket_window(code) {
   ...
   jQuery(function() {
	...
	function check_org_password()
	{
		var main_cat = jQuery('#protection_id').val();
		var password = jQuery('#org_password').val();
	...
	*/
	...
	function delete_my_event(event_id)
	{	var event_id = trim(event_id);
		if(event_id)
		{
			...
			jQuery.post('ajax/delete_my_event.php?',{ event_id: event_id }, function() {	
			jQuery('#message').fadeOut('slow',function(){
				  jQuery('#message').html('<span class=\"Redcolor\"><strong>Event has been deleted successfully</strong></span>').fadeIn('slow',function(){				
				  ajxpage('ajax/ajx_event_created_by_me.php?','my_event_block','fade','padding-top:60px; padding-left:20px;');				
				});
			  });
			});
			}
	...
	function delete_my_organization(org_id)
	{	var org_id = trim(org_id);
		if(org_id)
		{			 
			if(confirm("Do you really want delete this Organization ? \nThis action will delete all the data related with this organization & can not be reverted back !!")){
			jQuery.post('ajax/delete_my_organization.php?',{ org_id: org_id }, function() {	
			jQuery('#message').fadeOut('slow',function(){
				  jQuery('#message').html('<span class=\"Redcolor\"><strong>Organization has been deleted successfully</strong></span>').fadeIn('slow',function(){				
				  ajxpage('ajax/ajx_org_created_by_me.php?','my_organization_block','fade','padding-top:60px; padding-left:20px;');				
				});
			  });
			});
			}
